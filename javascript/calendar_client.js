document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderCalendar() {
        ...
        for (let i = 1; i <= lastDay.getDate(); i++) {
            ...
            let eventsForDay = appointments.filter((app) => app.schedule_date === dateStr);
            
            if (eventsForDay.length > 0) {
                eventsForDay.forEach(event => {
                    let eventDiv = document.createElement("div");
                    eventDiv.classList.add("event");
                    eventDiv.textContent = "📌 " + event.staff_name;
                    ...
                    eventDiv.addEventListener("mouseover", function () {
                        ...
                        stickyNote.textContent = `${event.staff_name} ${formattedTime}`;
